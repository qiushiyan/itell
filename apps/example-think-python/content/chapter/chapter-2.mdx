...
In many cases, you don't know the type of `a` beforehand, but you still want to concatenate it with a string without causing an error. In this case, you can use the `str()` function to convert `a` to a string.
