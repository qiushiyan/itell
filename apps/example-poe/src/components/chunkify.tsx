...
export const Chunkify = ({ children } : ChunkifyProps) => {
  ...
  */
  ...
  const handleSubmitClick = async () => {
    ...
    setIsLoading(true);
    const response = await getQAScore({input:inputValue, chapter:qObj[randomQuestionChunk].sectionId.split("-")[0], section:qObj[randomQuestionChunk].sectionId.split("-")[1], subsection:qObj[randomQuestionChunk].subsection.toString()});
          ...
          {index === currentChunk && index < chunks.length - 1 && (
             <div>
              ...
              {qObj[randomQuestionChunk].subsection === index ? (
                <div className={`flex justify-center items-center flex-col py-5 px-6 gap-2 
                                                border-4 my-6 rounded-md max-w-4xl mx-auto ${isShaking ? 'shake border-red-400' : isFlashingYellow ? 'border-yellow-400' : 'border-blue-400'}`}>
                  ...
                  {isPerfect < 2 && (<div className="flex justify-center items-center flex-col text-xs m-2">
                    {isPerfect == 0 && (<><p className="text-red-400"><b>iTELL AI says:</b> You likely got a part of the answer wrong. Please try again.</p>
                                          <p><u>If you believe iTELL AI has made an error, you can click on the "Skip this question" button to skip this question.</u> If you would like to help improve iTELL, please click on the feedback icons on the top right side of this box to give us feedback on this question.</p></>)}
                    {isPerfect == 1 && (<p className="text-yellow-600"><b>iTELL AI says:</b> You may have missed something, but you were generally close. You can click on the button below to continue reading or try again with a different response. </p>)} 
                    
                  </div>)}

                 {isPerfect == 2 ? (
                    <div className="flex items-center flex-col max-w-2xl">
                      <p className="text-xl2 text-emerald-600 text-center">Your answer was CORRECT!</p>
                      <p className="text-sm">
                        Click on the button below to continue reading. Please use the thumbs-up
                        or thumbs-down icons on the top right side of this box if you have any
                        feedback about this question that you would like to provide before you
                        continue reading.
                      </p>
                    </div>
                  ) : (
                  ...
                  {isPerfect === 2 ? (
                    <div className="flex justify-center items-center flex-row">
                      ...
                      <div className="flex justify-center items-center flex-row">
                        ...
                        >
                          {isLoading && (<Spinner className="inline" /> )}{isPerfect !== 1 ? "Submit" : "Resubmit"}
                        </button>
                        {isPerfect < 2 && (
                          <button 
                            className={cn(
                            ...
                            onClick={handleNextChunk}
                          >
                            {isPerfect === 1 ? "Click Here to Continue Reading" : "Skip this question"}
                          </button>
                        )}
                      </div>
                  )}
