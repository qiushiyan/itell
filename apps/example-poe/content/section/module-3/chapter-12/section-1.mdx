...
The aggregate demand and aggregate supply diagram in **Figure 12.3** shows two aggregate supply curves. We draw the original upward sloping aggregate supply curve (SRAS0) is a short-run or Keynesian AS curve. The vertical aggregate supply curve (LRASn) is the long-run or neoclassical AS curve, which is located at potential GDP. The original aggregate demand curve, labeled AD0, so that the original equilibrium occurs at point E0, at which point the economy is producing at its potential GDP.
